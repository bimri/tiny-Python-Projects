...
def test_one():
    ...
    assert out.strip() == 'You are bring chips.'


def test_two():
    """two items"""
    out = getoutput(f'{prg} soda "french fries')
    assert out.strip() == 'You are bring soda and french fries.'


def test_more_than_two():
    """more than two items"""
    arg = '"potato chips" coleslaw cupcakes "French silf pie'
...
def test_two_sorted():
    ...
    assert out.strip() == 'You are bring candy and soda.'
